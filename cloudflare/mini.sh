	...
	echo "Connection Complete"
    	...
    	if [ $CS -ne 0 ]
    		...
    		then
    			...
    			sleep 2
        		curl --header 'X-Auth-Token: OURKEY' --header 'X-Auth-Secret: OURKEYX' -H "Content-Type: application/json" -d '{"service":"xyzmino","status":"major-outage"}' -v http://status.digitalhazards.net/api/v1/services/set_status > /dev/null 2>&1
         		 stop_spinner $?
    		elif [ $CS -eq 0 ]
    		then
    			start_spinner '$SERVER $ON'
        		sleep 2
        		curl --header 'X-Auth-Token: OURKEY' --header 'X-Auth-Secret: OURKEYX' -H "Content-Type: application/json" -d '{"service":"xyzmino","status":"operational"}' -v http://status.digitalhazards.net/api/v1/services/set_status > /dev/null 2>&1
        		 stop_spinner $?
    	fi
